-- Baseline migration: captures the full schema as it existed before the first
-- recorded migration (20250923124905_add_auto_hide_child_tasks).
-- The project was originally set up via `prisma db push` with no migration
-- history. This file establishes the baseline so future `migrate dev` runs
-- have a clean, replayable history.
--
-- Intentionally excludes everything added by the recorded migrations that
-- follow this one:
--   - autoHideChildTasks column (added by 20250923124905)
--   - lastLoginAt column (added by 20260516170536)
--   - S3CleanupJob table (added by 20260518010000)
--   - Performance indexes on Project/Task/TaskAsset/Worklog (added by 20260518000000)
--   - CASCADE deletes on TaskAsset.taskId, Worklog.taskId, Task.projectId
--     (added by 20260516000000 and 20260516000001; baseline uses RESTRICT)

-- CreateEnum
CREATE TYPE "RecurrenceDuration" AS ENUM ('ONE_MONTH', 'ONE_YEAR', 'CUSTOM', 'CONTINUOUS');

-- CreateEnum
CREATE TYPE "RecurrenceFrequency" AS ENUM ('DAILY', 'WEEKLY', 'MONTHLY', 'ANNUALLY');
...
-- CreateIndex (these two were present before the 20260518000000 index migration)
CREATE INDEX "Task_parentId_idx" ON "Task"("parentId");
CREATE INDEX "Task_originalEventId_idx" ON "Task"("originalEventId");
...
-- AddForeignKey
ALTER TABLE "Task" ADD CONSTRAINT "Task_originalEventId_fkey" FOREIGN KEY ("originalEventId") REFERENCES "Task"("id") ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey: RESTRICT (CASCADE added later by 20260516000001)
ALTER TABLE "Task" ADD CONSTRAINT "Task_projectId_fkey" FOREIGN KEY ("projectId") REFERENCES "Project"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey: RESTRICT (CASCADE added later by 20260516000000)
